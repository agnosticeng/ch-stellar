...
impl RecordBatchExt for RecordBatch {
    fn get_column<T: 'static>(&self, col_name: &str) -> Result<&T, ArrowExtError> {
        let col = self.column_by_name(col_name).ok_or(ArrowExtError::ColumnNotFound(col_name.to_string()))?;
        let arr = col.as_any().downcast_ref().ok_or(ArrowExtError::CannotDowncastColumn(col_name.to_string()));
        arr
    }
}
...
impl ArrayRefExt for ArrayRef {
    fn as_array<T: 'static>(&self) -> Result<&T, ArrowExtError> {
        self.as_any().downcast_ref().ok_or(ArrowExtError::CannotDowncastArray)
    }
}