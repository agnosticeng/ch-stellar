...
use stellar_xdr::next::{Limits,Type,TypeVariant,Limited};
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            for input_batch in reader {
                ...
                let it = izip!(type_col, data_col) 
                    .map(|(_type, data)| -> Result<Type> {
                        let type_name = str::from_utf8(_type.unwrap()).context("type lust be valid utf8")?;
                        let type_variant = TypeVariant::from_str(type_name).context("failed to decode XDR data")?;
                        let mut xdr_reader = Limited::new(
                            data.unwrap(), 
                            Limits::depth(DEFAULT_XDR_RW_DEPTH_LIMIT)
                        );
